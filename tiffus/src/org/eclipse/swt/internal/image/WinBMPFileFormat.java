 ...
 */
int unloadData(ImageData image, byte[] data, NewOutputStream out, int comp) {
	int totalSize = 0;
	try {
		if (comp == 0)
 ...
 * Method modified to use the passed data if it is not null.
 */
int unloadDataNoCompression(ImageData image, byte[] data, NewOutputStream out) {
	int bmpBpl = 0;
	try {
		int bpl = (image.width * image.depth + 7) / 8;
...
void unloadIntoByteStream(ImageLoader loader) {
	...
	NewByteArrayOutputStream out = new NewByteArrayOutputStream();
