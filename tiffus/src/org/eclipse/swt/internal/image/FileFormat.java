...
import hu.newphrox.java.io.NewOutputStream;
...
public static FileFormat newFileFormat(int format) throws ClassNotFoundException{
	...
	switch (format) {
		...
		case SWT.IMAGE_BMP_RLE:
			return new WinBMPFileFormat(); 
		case SWT.IMAGE_GIF:
			return new GIFFileFormat(); 
		case SWT.IMAGE_ICO:
			return new WinICOFileFormat(); 
		case SWT.IMAGE_JPEG:
			return new JPEGFileFormat(); 
		case SWT.IMAGE_PNG:
			return new PNGFileFormat(); 
		case SWT.IMAGE_TIFF:
			return new TIFFFileFormat(); 
		case SWT.IMAGE_OS2_BMP:
			return new OS2BMPFileFormat();
		default:
			return new WinBMPFileFormat();
	}
...
public static ImageData[] load(InputStream is, int format, ImageLoader loader) {
	FileFormat fileFormat = null;
	LEDataInputStream stream = new LEDataInputStream(is);
	boolean isSupported = false;	
	try {
		fileFormat = newFileFormat(format);
 ...
 */	
public static void save(NewOutputStream os, int format, ImageLoader loader) {
	if (format < 0 || format >= FORMATS.length) SWT.error(SWT.ERROR_UNSUPPORTED_FORMAT);
	if (FORMATS[format] == null) SWT.error(SWT.ERROR_UNSUPPORTED_FORMAT);
