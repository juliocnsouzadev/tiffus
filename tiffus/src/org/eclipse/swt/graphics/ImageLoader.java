...
import org.eclipse.swt.main.SWTException;
...
public ImageData[] load(InputStream stream,int format) {
	if (stream == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
	reset();
	data = FileFormat.load(stream, format, this);
	return data;
}
 ...
 * </ul>
 */
public void save(NewOutputStream stream, int format) {
	if (stream == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
