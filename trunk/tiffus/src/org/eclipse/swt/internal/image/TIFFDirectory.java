...
final class TIFFDirectory {
	...
	static final short TAG_T6Options = 293;
...
void decodePixels(ImageData image) throws IOException {
	...
	for (int i = 0; i < length; i++) {
		...
		if (compression == COMPRESSION_NONE) {
		...
		} else if (compression == COMPRESSION_CCITT_3_1) {
		  ...
		  else if ( compression == COMPRESSION_CCITT_3_2) { 
			int fillOrder = 1;
			long tiffT4Options = 0;
			TIFFJaiFaxDecoder decoder = new TIFFJaiFaxDecoder(fillOrder, image.width, image.height);
			//stream.readFully(src data, 0, byteCount);
            decoder.decode2D(imageData, data, 0, image.height,tiffT4Options);
            destIndex += data.length;
            
            for (i=0; i<image.palette.colors.length; i++){
            	image.palette.colors[i].red^=0xFF;
	            image.palette.colors[i].green^=0xFF;
	            image.palette.colors[i].blue^=0xFF;
            }
            
        } //ccit group 4 2d
		  else if ( compression == COMPRESSION_CCITT_4_2) { 
			int fillOrder = 1;
			long tiffT6Options = 0;
			TIFFJaiFaxDecoder decoder = new TIFFJaiFaxDecoder(fillOrder, image.width, image.height);
			//stream.readFully(src data, 0, byteCount);
            decoder.decodeT6(imageData, data, 0, image.height,tiffT6Options);
            destIndex += data.length;
	        ...
	        try{
	            ...
	            System.arraycopy(o.toByteArray(), 0, imageData, destIndex, o.size());
	...
	for (int offset = 0; offset < buffer.length; offset += IFD_ENTRY_SIZE) {
		...
		switch (tag) {
			...
			case TAG_T6Options: {
				t6Options = 0;
				if (type != TYPE_LONG) SWT.error(SWT.ERROR_INVALID_IMAGE);
				t6Options = getEntryValue(type, buffer, offset);
				if ((t6Options & 0x1) == 1) {
				}
				break;
			}
